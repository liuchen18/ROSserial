...
void Callback(const geometry_msgs::Twist& cmd_vel){
  ...
  int delta_encoder_left_temp;
  int delta_encoder_right_temp;
  if(angular_temp>0){
    int delta_encoder_left_temp = WheelDistance / 0.039 * 390 * 4;//(linear_temp - 0.5 * (WheelDistance * angular_temp));// * encoder_sampling_time / speed_ratio;
    int delta_encoder_right_temp = - WheelDistance / 0.039 * 390 * 4;   //(linear_temp + 0.5 * (WheelDistance * angular_temp));// * encoder_sampling_time / speed_ratio;
  }
  else{
    int delta_encoder_left_temp = - WheelDistance / 0.039 * 390 * 4;//(linear_temp - 0.5 * (WheelDistance * angular_temp));// * encoder_sampling_time / speed_ratio;
    int delta_encoder_right_temp = WheelDistance / 0.039 * 390 * 4;   //(linear_temp + 0.5 * (WheelDistance * angular_temp));// * encoder_sampling_time / speed_ratio;
...
int main(int argc, char **argv){
  ros::init(argc, argv, "serial_biaoding_node");
  ros::NodeHandle nh;
   ...
   // ROS_INFO("send_updat_flag: %d", send_update_flag);
    ...
    if(1){
      ...
      if(p){
        ...
        for(int i = 0; i < p; i++){
          if(serial_data.data[i] == 255){
            if(i < p - 1){
              if(serial_data.data[i+1] == 225){
                readbuf[0] = serial_data.data[i];
                empty_flag = 1;  
              }
              else{
                break;
...
/*
   ...
   // std_msgs::UInt8MultiArray  serial_data;
      ...
      if(p!=0){
        while (bigNum<p&&num<11){
           readbuf.data[num++]=serial_data.data[bigNum];
           if (flag==0){
              if (serial_data.data[bigNum]==255){
                 flag=1;
              }
           }
           else{//flag==1
